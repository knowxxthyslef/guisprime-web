...
import {  ReportePatronesVigentesSinTrabajadores, busquedaRequestModel } from 'src/app/comun/model/administracion.model';
...
@Component({
  selector: 'app-patrones-vigentes-sin-trabajadores',
  templateUrl: './patrones-vigentes-seis-meses.component.html',
  styleUrls: ['./patrones-vigentes-seis-meses.component.scss']
})
export class PatronesVigentesSeisMesesComponent extends GeneralComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cifraSubDelegaciones = new SummaryColoredCardModel({
      ...
      description: 'Total OOAD y subdelegaci&oacute;n',
      color: '#e7eafd',
      numericValue: 100
    });
  ...
  onPaged(){
    ...
    this.administrationService.getJSONPatronesSinTrabajadores(this.fetchRequest)
    .subscribe({
        next: (result) => {
          console.log(result)
          this.pageData.init(result);
          this.data = DeserializeArray(result.content, ReportePatronesVigentesSinTrabajadores)
        },
        error: (e) => {
